<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="赞助企业" prop="zanzhuqiye">
          <el-select v-model="ruleForm.zanzhuqiye" placeholder="请选择赞助企业">
            <el-option
                v-for="(item,index) in zanzhuqiyeOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
...
export default {
  ...
  methods: {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/zanzhuqiye/qiyemingcheng`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.zanzhuqiyeOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
	...
	this.ruleForm.huodongneirong = this.ruleForm.huodongneirong.replace(reg,'../../../ssml1ro4/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
