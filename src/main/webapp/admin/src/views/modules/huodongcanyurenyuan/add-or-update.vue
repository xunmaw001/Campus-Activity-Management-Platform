    ...
    >
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="参与人员" prop="canyurenyuan">
                <editor 
              ...
              <div v-else>
                <el-form-item v-if="ruleForm.canyurenyuan" label="参与人员" prop="canyurenyuan">
                    <span v-html="ruleForm.canyurenyuan"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    info(id) {
      this.$http({
        url: `huodongcanyurenyuan/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	this.ruleForm.canyurenyuan = this.ruleForm.canyurenyuan.replace(reg,'../../../ssml1ro4/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `huodongcanyurenyuan/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.huodongcanyurenyuanCrossAddOrUpdateFlag = false;
                  this.parent.search();
